...
public class PPOrderRepositoryImpl implements PPOrderRepositoryCustom{
	...
	public Page<PPOrderDTO> findAll(Pageable pageable) {
		...
		if (total.longValue() > 0) {
			Query query = entityManager.createNativeQuery("select p.id id, s.supplierCode supplierCode, p.no, p.date date, p.totalAmount totalAmount " + sql + " order by p.date ", "PPOrderDTO");
